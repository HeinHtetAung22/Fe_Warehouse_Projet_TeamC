...
<div class="main-content" style="overflow-y: auto;">
	<div class="top-bar mb-3">
		<form id="searchForm"
			action="${pageContext.request.contextPath}/SearchSupplierInfo/1"
			method="get">
			<div
						...
						<c:choose>
									...
									<td colspan="5" class="text-center py-4">
										<div class="d-flex flex-column align-items-center">
											...
											<a href="${pageContext.request.contextPath}/SupplierInfo"
												...
												class="fas fa-arrow-left me-1"></i> Back to all suppliers
											</a>
										</div>
									</td>
								</tr>
							...
							<c:otherwise>
								<c:forEach var="supplier" items="${suppliers}"
									varStatus="status">
									<tr>
										<td>SUP${(currentPage - 1) * 10 + status.index + 1}</td>
										<!-- 
                                          ========================================
				...
				<div class="d-flex justify-content-center mt-3">
					<nav aria-label="Page navigation">
						<ul class="pagination pagination-sm">
							...
							<!-- Update the pagination links to include sorting parameters -->
							<c:forEach begin="1" end="${totalPages}" var="i">
								<li class="page-item ${currentPage == i ? 'active' : ''}">
									<c:choose>
										<c:when test="${not empty searchQuery}">
											<a class="page-link"
												href="${pageContext.request.contextPath}/SearchSupplierInfo/${i}?searchQuery=${searchQuery}&sortField=${param.sortField}&sortDirection=${param.sortDirection}">
												${i} </a>
										</c:when>
										<c:otherwise>
											<a class="page-link"
												href="${pageContext.request.contextPath}/SupplierInfo/${i}?sortField=${param.sortField}&sortDirection=${param.sortDirection}">
												${i} </a>
										</c:otherwise>
									</c:choose>
								</li>
...
<script>
	...
	function applySort(field, direction) {
	    ...
	    let actionUrl;
	    if (isSearch) {
	        actionUrl = basePath + "/SearchSupplierInfo/1?searchQuery=" + encodeURIComponent(searchQuery) + 
	                   "&sortField=" + field + "&sortDirection=" + direction;
	    } else {
	        actionUrl = basePath + "/SupplierInfo/1?sortField=" + field + 
